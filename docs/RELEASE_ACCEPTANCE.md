...
| 18 | Implemented | Runtime emergency stop invalidates every live session and approval, releases input, and clears session-scoped media handles. Studio exposes start, pause, resume, stop, and global emergency-stop controls. Cancellable media/editor processes are covered by cancellation, runtime, UI, and portable-backend tests. | The eight-hour physical input run remains criterion 10's release qualification. |
