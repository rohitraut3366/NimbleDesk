...
| 18 | Partial | Runtime emergency stop invalidates every live session and approval, releases input, and clears session-scoped media handles. Studio exposes start, pause, resume, stop, and global emergency-stop controls. A companion liveness-pipe watchdog releases common keys and buttons after daemon death before service-manager restart. Cancellable media/editor processes are covered by cancellation, runtime, watchdog, UI, and portable-backend tests. | Add the configurable global pause shortcut and tray-menu stop. The eight-hour physical input run remains criterion 10's release qualification. |
