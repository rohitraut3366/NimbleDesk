...
| 3 | Qualification required | `src/nimbledesk/testing/fixture_app.py` provides a state-file and event-log oracle. `nimbledesk fixture-contract` runs bounded capture, pointer restoration, keyboard input, and named semantic actions while checking real postconditions. macOS native mode now uses ScreenCaptureKit/Core Graphics capture and CGEvent input rather than PyAutoGUI. Fixture and contract tests verify the harness. | Run and retain physical contract reports on macOS, Windows, GNOME Wayland/X11, and KDE. |
