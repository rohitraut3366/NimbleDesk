...
| 18 | Partial | Runtime emergency stop invalidates every live session and approval, releases input, and clears session-scoped media handles. Studio exposes start, pause, resume, stop, and global emergency-stop controls. A companion liveness-pipe watchdog releases common keys and buttons after daemon death before service-manager restart. The daemon also supervises a tray console with a validated installation-time shortcut, authenticated pause-all and emergency-stop actions, atomic status, and parent-liveness shutdown. Cancellable media/editor processes are covered by cancellation, runtime, watchdog, safety-console, UI, and portable-backend tests. | Implement and qualify the XDG GlobalShortcuts portal path for native Wayland sessions, then exercise shortcut and tray stop on every physical target. The eight-hour physical input run remains criterion 10's release qualification. |
