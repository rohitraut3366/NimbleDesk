...
| 16 | Implemented | `src/nimbledesk/daemon/policy.py`, `runtime.py`, and `audit.py`; `tests/test_policy.py`, `tests/test_runtime.py`, and adapter approval tests verify policy and audit routing. Clipboard access has separate host/session gates and bounded redacted reads; writes and allowlisted application launches require exact approval. | Physical input runs are covered by criterion 3. |
