...
| 19 | Qualification required | DaVinci, semantic vision, and application adapters execute in bounded subprocesses. macOS uses a deny-by-default sandbox, Linux uses Bubblewrap, and bundled Windows workers use AppContainer. All receive declared paths, opt-in network access, deadlines, response limits, and bounded child processes and memory. `nimbledesk windows-adapter-contract` verifies Windows path, process, memory, and network-capability isolation and is mandatory in Windows bundle builds. | Physically exercise the malicious-provider fixtures on every supported OS and retain the Windows bundle contract report. |
...
corpus reports as one machine-readable gate. Signing and physical adapter evidence remain separate
release gates.
