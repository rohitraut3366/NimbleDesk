...
| 19 | Partial | DaVinci, semantic vision, and application adapters execute in bounded subprocesses. macOS third-party adapters use a tested deny-by-default sandbox; Linux requires Bubblewrap and disables networking by default. Windows sandboxed workers use a maximum-privilege-stripped token plus a one-process, 512 MiB Job Object with kill-on-close containment. | Add an AppContainer filesystem/network broker so Windows workers receive only declared paths and network access, then physically exercise malicious-provider fixtures on every OS. |
